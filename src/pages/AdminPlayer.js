...
const AdminPlayer = ({ doGetWholePlayer, doGetWholeLeague, UpdatePlayer, AddPlayer, DeletePlayer, doGetWholeTeam, ChangePlayerTeam, players, teams, refresh }) => {
    ...
    const rows1 = players.map((data, i) => (
        <TableRow key={data.firstname} onClick={() => { setCurData(data._id); setFirstName(data.firstname); setLastName(data.lastname); setPrice(data.price) }}>
    ...
    return (
        <StyledContainer>
            <Box style={{ margin: "auto" }}>
                ...
                <Button color='secondary' variant="contained" className='text-center' onClick={() => { AddPlayer(firstname, lastname, price) }}>
                    Add
                </Button> &nbsp;&nbsp;
                <Button color='secondary' variant="contained" className='text-center' onClick={() => { UpdatePlayer(curdata, firstname, lastname, price) }}>
                    Update
                </Button>
            </Box>
